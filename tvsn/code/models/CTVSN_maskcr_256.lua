...
function DCGAN.create_netD(opts)
  ...
  -- 3 x 256 x 256
	-- en_conv1 = nn.LeakyReLU(0.2, true)(cudnn.SpatialBatchNormalization(64)(cudnn.SpatialConvolution(3,64,4,4,2,2,1,1)(input_im))):annotate{name='feat1'}
  -- 16 x 128 x 128
  ...
  -- 512 x 4 x 4
	...
	-- cudnn.SpatialConvolution(nInputPlane, nOutputPlane, kW, kH, dW, dH, padW, padH, groups)

	-- input: 3 x 256 x 256
	local en_conv1 = nn.LeakyReLU(0.2, true)(cudnn.SpatialBatchNormalization(64)(cudnn.SpatialConvolution(3,32,4,4,2,2,1,1)(input_im))):annotate{name='feat1'}
  -- en_conv1 = 32 x 128 x 128
  -- en_conv2 = 64 x 64 x 64
	local en_conv2 = nn.LeakyReLU(0.2, true)(cudnn.SpatialBatchNormalization(64)(cudnn.SpatialConvolution(32,64,4,4,2,2,1,1)(en_conv1))):annotate{name='feat2'}
	-- en_conv3 = 128 x 4 x 4
	local en_conv3 = nn.LeakyReLU(0.2, true)(cudnn.SpatialBatchNormalization(128)((cudnn.SpatialConvolution(64,128,40,40,8,8,0,0)(en_conv2)))):annotate{name='feat3'}
  ...
  -- (deleted) en_conv5 = 512 x 4 x 4
	...
	local out = nn.View(1)(nn.Sigmoid()(cudnn.SpatialConvolution(128,1,4,4)((en_conv3))))
