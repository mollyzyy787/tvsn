...
opt = lapp[[
  --dataset						(default 'doafn')
	--split							(default 'train')
  --saveFreq          (default 20)
  --modelString       (default 'CDOAFN2_SYM')
  -g, --gpu           (default 0)
...
for t = epoch+1, opt.maxEpoch do
  ...
  net:training()
	for i = 1, opt.iter_per_epoch do
		...
		-- calculate contour from batch_im_out
		batch_map2 = torch.sum(batch_im_out,2) --sum over channeal, background sum to 3, other < 3
		batch_map2 = torch.floor(batch_map2:mul(1/3)) --background change to 1, other to 0
		batch_map2 = batch_map2:add(-1):mul(-1) --bachground change to 0, other to 1
		-- make it [0, 1] -> [-1, 1]
